...
# In this exercise we will develop a 2-layer neural network with fully-connected layers to
# perform classification, and test it out on the CIFAR-10 dataset. In general, ReLU activation works the best.
# So ReLU instead of sigmoid will be used in the assignments afterwards.
...
# We will use the class `TwoLayerNet` in the file `lib/classifiers/neural_net.py` to represent instances of our network.
# The network parameters are stored in the instance variable `self.params` where keys are string parameter names and values are
# numpy arrays. Below, we initialize toy data and a toy model that we will use to develop your implementation.
...
# Open the file `lib/classifiers/neural_net.py` and look at the method `TwoLayerNet.loss`.
...
# # Backward pass
# Implement the rest of the function. This will compute the gradient of the loss with respect to the variables `W1`, `b1`, `W2`,
# and `b2`. Now that you (hopefully!) have a correctly implemented forward pass, you can debug your backward pass using a numeric
# gradient check:
...
#
grid_search = [(x, y, z)
               for x in hidden_sizes for y in learning_rates for z in regs]
